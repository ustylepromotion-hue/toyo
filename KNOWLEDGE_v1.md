...
| ファイル | サイズ | 用途 | デプロイ対象 |
|---|---|---|---|
| `index.html` | 7.2K | メインHTML（SEO/OGP/構造化データ/favicon込み） | ✅ |
...
3. **Google Search Console登録**: 「豊橋まちあるきスタンプ」でインデックスされるまで待つ（通常数日〜2週間）
4. **PageSpeed Insights**: 95点以上出るはず（静的・画像1枚のみの軽量構成）
...
以上、ver1 ナレッジとして保管。次回はこのファイルを読めば即復帰可能。
